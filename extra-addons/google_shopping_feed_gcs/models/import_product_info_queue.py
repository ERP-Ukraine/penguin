...
class ImportProductInfoQueue(models.Model):
    _name = "import.product.info.queue.gcs"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = "Import Product Info. Queue GCS"
    _order = "name desc"
    ...
    import_product_info_queue_lines = fields.One2many("import.product.info.queue.line.gcs",
                                                    "import_product_info_queue_id",
                                                    string="Import Product Info. Queue Lines")
    ...
    @api.depends("import_product_info_queue_lines.state")
    ...
    def _compute_queue_line_records(self):
        for product_queue in self:
            queue_lines = product_queue.import_product_info_queue_lines
    ...
    def create(self, vals):
        sequence_id = self.env.ref("google_shopping_feed_gcs.google_shopping_import_product_info_queue_seq").ids
        if sequence_id:
            sequence_name = self.env["ir.sequence"].browse(sequence_id).next_by_id()
        else:
            sequence_name = "/"
        vals.update({"name": sequence_name or ""})
        return super(ImportProductInfoQueue, self).create(vals)
    ...
    def generate_sendone_notification(self, message):
        ...
        bus_bus_obj._sendone(self.env.user.partner_id, "google_shopping_import_product_info_queue_gcs", {
                             "title": "Google Shopping Connector",
                             "message": message})


class ImportProductInfoQueueLines(models.Model):
    _name = "import.product.info.queue.line.gcs"
    _description = "Import Product Info. Queue Line GCS"
    ...
    import_product_info_queue_id = fields.Many2one("import.product.info.queue.gcs", "Queue ID", required=True,
                                                   ondelete="cascade", copy=False)
    gs_instance_id = fields.Many2one('google.shopping.feed.instance',
                                     related="import_product_info_queue_id.gs_instance_id",
                                     store=True,
    ...
    log_tracker_lines = fields.One2many("google.shopping.feed.log.line.tracker", "import_product_info_queue_line_id",
