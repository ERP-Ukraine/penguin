...
class UpdateProductDataQueue(models.Model):
    _name = "update.product.data.queue.gcs"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = "Update Product Data Queue GCS"
    _order = "name desc"
    ...
    update_product_data_queue_lines = fields.One2many("update.product.data.queue.line.gcs", 
                                                      "update_product_data_queue_id", string="Product Queue Lines")
    ...
    @api.depends("update_product_data_queue_lines.state")
    ...
    def _compute_queue_line_records(self):
        for product_queue in self:
            queue_lines = product_queue.update_product_data_queue_lines
    ...
    def create(self, vals):
        sequence_id = self.env.ref("google_shopping_feed_gcs.google_shopping_update_product_queue_data_seq").ids
        if sequence_id:
            sequence_name = self.env["ir.sequence"].browse(sequence_id).next_by_id()
        else:
            sequence_name = "/"
        vals.update({"name": sequence_name or ""})
        return super(UpdateProductDataQueue, self).create(vals)
    ...
    def generate_sendone_notification(self, message):
        ...
        bus_bus_obj._sendone(self.env.user.partner_id, "google_shopping_update_product_data_queue_gcs", {
                             "title": "Google Shopping Connector",
                             "message": message})
        
    
class UpdateProductDataQueueLine(models.Model):
    _name = "update.product.data.queue.line.gcs"
    _description = "Update Product Data Queue Line GCS"
    ...
    update_product_data_queue_id = fields.Many2one("update.product.data.queue.gcs", "Queue ID", required=True,
                                                   ondelete="cascade", copy=False)
    gs_instance_id = fields.Many2one('google.shopping.feed.instance', 
                                     related="update_product_data_queue_id.gs_instance_id",
                                     store=True,
    ...
    log_tracker_lines = fields.One2many("google.shopping.feed.log.line.tracker", "update_product_data_queue_line_id",
