...
class UpdateProductInventoryInfoQueue(models.Model):
    _name = "update.product.inventory.info.queue.gcs"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = "Update Product Inventory Info. Queue GCS"
    _order = "name desc"
    ...
    update_product_inventory_info_queue_lines = fields.One2many("update.product.inventory.info.queue.line.gcs", 
                                                      "update_product_inventory_info_queue_id", 
    ...
    @api.depends("update_product_inventory_info_queue_lines.state")
    ...
    def _compute_queue_line_records(self):
        for product_queue in self:
            queue_lines = product_queue.update_product_inventory_info_queue_lines
    ...
    def create(self, vals):
        sequence_id = \
            self.env.ref("google_shopping_feed_gcs.google_shopping_update_product_inventory_info_queue_seq").ids
        if sequence_id:
            sequence_name = self.env["ir.sequence"].browse(sequence_id).next_by_id()
        else:
            sequence_name = "/"
        vals.update({"name": sequence_name or ""})
        return super(UpdateProductInventoryInfoQueue, self).create(vals)
    ...
    def generate_sendone_notification(self, message):
        ...
        bus_bus_obj._sendone(self.env.user.partner_id, "google_shopping_update_product_inventory_info_queue", {
                             "title": "Google Shopping Connector",
                             "message": message})
        
    
class UpdateProductInventoryInfoQueueLine(models.Model):
    _name = "update.product.inventory.info.queue.line.gcs"
    _description = "Update Product Inventory Info. Queue Line GCS"
    ...
    update_product_inventory_info_queue_id = fields.Many2one("update.product.inventory.info.queue.gcs", "Queue ID", 
                                                             required=True, ondelete="cascade", copy=False)
    gs_instance_id = fields.Many2one('google.shopping.feed.instance', 
                                     related="update_product_inventory_info_queue_id.gs_instance_id",
                                     store=True,
                                     string="Shopping Instance")
    gs_account_id = fields.Many2one("google.shopping.feed.account", related="gs_instance_id.gs_account_id", store=True, 
                                    string="Shopping Account", help="Shopping account")
    product_inventory_request_data = fields.Text("Product Inventory Request Data")
    ...
    log_tracker_lines = fields.One2many("google.shopping.feed.log.line.tracker", 
                                        "update_product_inventory_info_queue_line_id",
