...
class WalleePaymentController(PaymentPortal):
...
class WalleePostPaymentController(PaymentPostProcessing):
    ...
    def poll_status(self):
        ...
        tx_ids_list = request.env['payment.transaction'].sudo().search([
        ...
        return super(WalleePostPaymentController, self).poll_status()
...
class WalleeController(http.Controller):   
    ...
    @http.route([_wallee_redirect_url], type='http', auth='public')
    ...
    @http.route([_success_url, _failed_url], type='http', auth='public')
    def wallee_form_feedback(self, txnId=None, **post):
        ...
        try:
            request.env['payment.transaction'].sudo()._handle_feedback_data('wallee', post)
        except Exception as e:
