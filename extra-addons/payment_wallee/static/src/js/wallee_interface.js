odoo.define('payment_wallee.wallee_interface', function (require) {
    ...
    const walleeMixin = {
        ...
        _processRedirectPayment: function (provider, acquirerId, processingValues){
            if (provider !== 'wallee' || provider == 'wallee' && processingValues['api_url']) {
    ...
    checkoutForm.include(walleeMixin);
    manageForm.include(walleeMixin);
...
//            this.$el.find('.o_wallee_interface_content').html(html);
            this.$el.find('div[name="o_wallee_interface_content"]').html(html);
        },
